...
### Phrase-Synced Caption (DEFAULT for voiceover videos)

Displays phrases synced to Whisper word timestamps. Shows 4-6 words at a time, swapping with a spring scale-in. This is the **recommended default** for body text with voiceover — replaces WordByWord for most scenes.

Requires `words` array in the audio manifest (generated by Whisper post-processing step).

```tsx
import { groupWordsIntoPhrases, getCurrentPhrase, buildEvenSpacedPhrases } from "../lib/whisper-captions";
...
interface PhraseSyncedCaptionProps {
  /** Word timestamps from manifest (Whisper output) */
  words?: WordTimestamp[];
  /** Fallback: raw text if no word timestamps available */
  text?: string;
  /** Scene duration in seconds (for fallback even-spacing) */
  durationSec?: number;
  /** Font size in px (default: 68) */
  fontSize?: number;
  /** Whether to show active word highlight (default: false) */
  highlightActive?: boolean;
}

const PhraseSyncedCaption: React.FC<PhraseSyncedCaptionProps> = ({
  words,
  text,
  durationSec = 5,
  fontSize = 68,
  ...
  // Build phrases from word timestamps or fallback to even spacing
  const phrases = React.useMemo(() => {
    if (words && words.length > 0) {
      return groupWordsIntoPhrases(words);
    }
    if (text) {
      return buildEvenSpacedPhrases(text, durationSec);
    }
    return [];
  }, [words, text, durationSec]);

  const current = getCurrentPhrase(phrases, frame, fps);
  if (!current.text) return null;

  // Spring scale-in on phrase change
  const phraseStartFrame = Math.round(current.start * fps);
  const elapsed = frame - phraseStartFrame;

  const scale = spring({
    frame: elapsed,
    fps,
    config: { damping: 15, stiffness: 250, mass: 0.4 },
  });
  ...
  // Active word highlight (optional)
    ...
    <div style={{
      ...
      <div style={{
        ...
        {highlightActive && current.words?.length ? (
          ...
          // Simple phrase display (default)
          <span style={{
            fontSize,
            fontWeight: 900,
            color: "#FFFFFF",
            textShadow: "0 2px 8px rgba(0,0,0,0.8), 0 0 40px rgba(0,0,0,0.4)",
            textAlign: "center",
            lineHeight: 1.2,
...
```tsx
// With Whisper timestamps (from enriched manifest)
<PhraseSyncedCaption words={manifest["scene-1-hook"].words} />

// With active word highlight
<PhraseSyncedCaption words={manifest["scene-1-hook"].words} highlightActive />

// Fallback (no Whisper — even-spaced)
<PhraseSyncedCaption text="Your baby can taste what mom eats" durationSec={4.2} />
```
