...
import { WhimsicalBackground, WhimsicalVariant } from "./components/WhimsicalBackground";
import { VideoConfig, PALETTES } from "./videos-config";

const BABY_TEXT_SHADOW = "0 2px 8px rgba(0,0,0,0.5), 0 0 20px rgba(0,0,0,0.3)";

// Per-variant accent color — used for badges, dots, pills, brand chip
// when in baby-facts mode. Picks a saturated tone from each palette so
// chips/dots stay visible on light pastel backgrounds.
const WHIMSICAL_ACCENTS: Record<WhimsicalVariant, string> = {
  blossom: "#FF6B9D",   // vibrant pink
  meadow:  "#52B788",   // forest green
  dawn:    "#FF8A5C",   // warm coral
  dream:   "#7B6BE8",   // deep periwinkle
  garden:  "#C45F6B",   // muted rose
  cloud:   "#6B6BD9",   // saturated periwinkle
  sunset:  "#E8633C",   // burnt orange
};
...
export const FactsVideo: React.FC<FactsVideoProps> = ({ videoId, config, manifest }) => {
  ...
  const palette = PALETTES[config.bgPalette ?? "purple"];
  ...
  const whimsicalVariant: WhimsicalVariant = config.whimsicalVariant ?? "blossom";
  // For baby-facts, derive accent from the whimsical variant — DON'T use the
  // dark tech `palette.accent` which would render purple/green chips on pastel.
  const sceneAccent = isBabyFacts ? WHIMSICAL_ACCENTS[whimsicalVariant] : palette.accent;
  const sceneTextHigh = isBabyFacts ? "#FFFFFF" : palette.textHigh;
  const scenePalette = { ...palette, accent: sceneAccent, textHigh: sceneTextHigh };

  return (
    <AbsoluteFill style={{ opacity: openFade, fontFamily: "'Inter', 'Helvetica Neue', Arial, sans-serif" }}>
      {/* HOOK — whimsical theme="hook" gradient for baby-facts, else corporate */}
      <Sequence from={0} durationInFrames={hookFrames}>
        {isBabyFacts ? (
          <WhimsicalBackground theme="hook" variant={whimsicalVariant} />
        ) : (
          <SharedBackground palette={palette} variant={bgVariant} />
        )}
        <Audio src={staticFile(`audio/${videoId}/hook.wav`)} />
        <HookScene config={config} palette={scenePalette} />
      </Sequence>

      {/* BODY — whimsical theme="body" gradient for baby-facts */}
      <Sequence from={hookFrames} durationInFrames={bodyFrames}>
        {isBabyFacts ? (
          <WhimsicalBackground theme="body" variant={whimsicalVariant} />
        ) : (
          <SharedBackground palette={palette} variant={bgVariant} />
        )}
        <Audio src={staticFile(`audio/${videoId}/body.wav`)} />
        <BodyScene config={config} palette={scenePalette} />
      </Sequence>

      {/* CTA — whimsical theme="cta" gradient for baby-facts */}
      <Sequence from={hookFrames + bodyFrames} durationInFrames={ctaFrames}>
        {isBabyFacts ? (
          <WhimsicalBackground theme="cta" variant={whimsicalVariant} />
        ) : (
          <SharedBackground palette={palette} variant={bgVariant} />
        )}
        <Audio src={staticFile(`audio/${videoId}/cta.wav`)} />
        <CTAScene config={config} palette={scenePalette} />
      </Sequence>

      {/* Brand tag — always visible (uses scenePalette so accent is whimsical for baby-facts) */}
      <Sequence from={15} durationInFrames={hookFrames + bodyFrames + ctaFrames - 15}>
        <BrandTag project={config.project} palette={scenePalette} />
      </Sequence>
    </AbsoluteFill>
  );
};
