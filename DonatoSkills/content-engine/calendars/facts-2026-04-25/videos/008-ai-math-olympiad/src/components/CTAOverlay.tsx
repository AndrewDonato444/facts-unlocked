...
export const CTAOverlay: React.FC = () => {
  ...
  // Fast slide-in for fast-pace video
  const slideProgress = spring({ frame: frame - 6, fps, config: { damping: 14, stiffness: 200, mass: 0.6 } });
  ...
  const pulse = interpolate(Math.sin((frame / 10) * Math.PI), [-1, 1], [0.90, 1.0]);
    ...
    <AbsoluteFill style={{
      ...
      <div style={{ transform: `translateY(${translateY}px)`, display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        <div style={{
          ...
          lineHeight: 1.2,
          textShadow: `0 0 40px rgba(245,158,11,0.55), 0 4px 20px rgba(0,0,0,0.80)`,
        }}>{CTA_TEXT}</div>
        <div style={{
