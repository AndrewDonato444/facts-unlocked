...
// AI Facts Unlocked: 009-ai-traffic-lights
// Voice: Kore (sharp, authoritative, fast)
...
const scenes = [
  {
    ...
    direction: "bold, punchy, urgent — land hard on 'twenty-six percent'",
  },
  {
    name: "scene-2",
    script: "Carnegie Mellon's Surtrac AI manages fifty-four intersections in Pittsburgh. Instead of fixed timing patterns, it reads live traffic every second and coordinates signals city-wide. The result: twenty-six percent less travel time, forty percent fewer red light stops, and twenty-one percent lower vehicle emissions.",
    direction: "confident, building awe — each number should land with weight",
  },
  {
    name: "scene-3",
    script: "Traffic lights have worked the same way for a century. That just changed.",
    direction: "definitive, final — like closing a chapter of history",
  },
  {
    name: "scene-4",
    script: "Did you know?",
...
async function main() {
  ...
  for (const scene of scenes) {
    ...
    try {
      ...
      console.log(`  -> ${filepath} (${durationSec.toFixed(2)}s)`);
    } catch {
