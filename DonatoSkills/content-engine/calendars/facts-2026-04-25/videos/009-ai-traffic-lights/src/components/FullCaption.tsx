...
const CaptionWord: React.FC<{ word: string; startFrame: number; endFrame: number; isActive: boolean; isPast: boolean; }> = ({ word, startFrame, endFrame, isActive, isPast }) => {
  ...
  const baseColor = isPast ? "rgba(240,253,249,0.38)" : isActive ? COLORS.tealBright : "rgba(240,253,249,0.82)";
  ...
  return (
    <span style={{
      ...
      filter: isActive ? `drop-shadow(0 0 ${glowIntensity}px ${COLORS.teal})` : "none",
      marginRight: 10,
      marginBottom: 4,
      fontWeight: isActive ? 900 : 700,
      textShadow: isActive
        ? `0 0 18px rgba(45,212,191,0.65), 0 2px 8px rgba(0,0,0,0.92)`
