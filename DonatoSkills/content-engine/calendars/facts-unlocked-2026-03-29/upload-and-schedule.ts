...
const VIDEO_DIRS: Record<string, string> = {
  "001": "001-vernix-caseosa",
  "002": "002-breast-milk-composition",
  "003": "003-lanugo-hair",
  "004": "004-payday-loan-apr",
  "005": "005-gig-economy-hidden-costs",
  "006": "006-zip-code-life-expectancy",
  "007": "007-deepseek-cost-shock",
  "008": "008-ai-voice-scams-surge",
  "009": "009-ai-synthetic-data",
};
...
async function main() {
  ...
  for (const item of items) {
    ...
    const publicId = `facts-unlocked/2026-03-29/${item.id}-${item.slug}`;
  ...
  calendarDoc.stats = { total: items.length, scheduled, failed, skipped };
  fs.writeFileSync(CALENDAR_PATH, JSON.stringify(calendarDoc, null, 2));
  ...
  const costData = {
    date: "2026-03-29",
    campaign: "facts-unlocked-2026-03-29",
    items: costItems,
    total_estimated_usd: parseFloat((costItems.reduce((sum, i) => sum + i.estimated_cost_usd, 0)).toFixed(2)),
  };
  fs.writeFileSync(path.join(costTrackerDir, "2026-03-29.json"), JSON.stringify(costData, null, 2));
  console.log(`\nCost estimate: $${costData.total_estimated_usd} (logged to cost-tracker/2026-03-29.json)`);
}
